...
export function useQuizAnswer() {
    ...
    const optionClass = (quiz: QuizQuestion | null, option: string) => {
        ...
        if (answerState === 'idle')
            return base + 'border-border text-foreground hover:border-primary hover:text-primary bg-background cursor-pointer'
        if (option === quiz?.correct_answer)
            return base + 'border-primary text-primary bg-primary/5 cursor-default'

        if (option === selected && answerState === 'wrong')
            return base + 'border-[#EF4444] text-[#EF4444] bg-[#EF4444]/5 cursor-default'
        return base + 'border-border text-muted-foreground cursor-default opacity-50'
    }
