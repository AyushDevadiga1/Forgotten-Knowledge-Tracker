...
from tracker_app.learning.sm2_memory_model import SM2Item, SM2Scheduler, MIN_EASE_FACTOR, MAX_EASE_FACTOR
...
class TestSM2EaseFactor(unittest.TestCase):
    ...
    def test_ease_factor_never_below_minimum(self):
        ...
        self.assertGreaterEqual(
            item.ease_factor, MIN_EASE_FACTOR, f"ease_factor {item.ease_factor} dropped below minimum {MIN_EASE_FACTOR}"
        )
    ...
    def test_ease_factor_never_above_maximum(self):
        ...
        self.assertLessEqual(
            item.ease_factor,
            MAX_EASE_FACTOR + 0.01,  # small float tolerance
    ...
    def test_ease_factor_decreases_on_low_quality(self):
        ...
        result = SM2Scheduler.calculate_next_interval(item, quality=2)
        self.assertLessEqual(result["ease_factor"], original_ef, "Low quality should decrease or keep ease factor")
    ...
    def test_ease_factor_increases_on_high_quality(self):
        ...
        result = SM2Scheduler.calculate_next_interval(item, quality=5)
        self.assertGreaterEqual(
            result["ease_factor"], MIN_EASE_FACTOR, "High quality should not drop ease factor below minimum"
        )
...
class TestSM2Intervals(unittest.TestCase):
    ...
    def test_interval_grows_after_repeated_success(self):
        ...
        for i in range(1, len(intervals)):
            self.assertGreaterEqual(intervals[i], intervals[i - 1], f"Interval should grow: {intervals}")
...
class TestSM2QualityBoundaries(unittest.TestCase):
    ...
    def test_quality_exactly_3_passes(self):
        ...
        self.assertGreaterEqual(result["repetitions"], 1, "Quality 3 should count as a pass (repetitions >= 1)")
    ...
    def test_quality_exactly_2_fails(self):
        ...
        self.assertLessEqual(result["repetitions"], 1, "Quality 2 should reset repetitions to 0 or 1 (failure mode)")
        # Interval must still be short
        self.assertLessEqual(result["next_interval_days"], 1, "Failed review must schedule next review within 1 day")
...
class TestSM2DatesAndScheduling(unittest.TestCase):
    ...
    def test_next_review_date_is_in_future(self):
        ...
        if nrd is not None:
            ...
            self.assertGreater(nrd, yesterday, "Next review date should not be in the past")
