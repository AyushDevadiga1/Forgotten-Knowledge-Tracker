...
def test_in_memory_subsumption_avoids_db(monkeypatch):
    ...
    assert cp._is_subsumed_single_word("cellular respiration", subsuming_phrases=phrases) is False
...
def test_promote_reuses_single_db_session(db, monkeypatch):
    ...
    monkeypatch.setattr("tracker_app.learning.learning_tracker.models.SessionLocal", item_maker)
