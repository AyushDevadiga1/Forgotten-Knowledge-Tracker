...
class TestAtomicClaim(ToastCooldownTestBase):
    ...
    def test_only_one_request_can_claim_a_prediction(self):
        ...
        with self.TestingSessionLocal() as db:
            self.assertIsNotNone(db.query(IntentPrediction).filter(IntentPrediction.id == pid).first().prompted_at)
    ...
    def test_endpoint_respects_already_claimed_row(self):
        ...
        with self.TestingSessionLocal() as db:
            db.execute(update(IntentPrediction).where(IntentPrediction.id == pid).values(prompted_at=datetime.utcnow()))
            db.commit()
        self.assertIsNone(self._get_recent())
