...
def update_memory_scores(ocr_keywords: Dict[str, Any], attention_score: float, audio_label: str):
    """Update memory scores for OCR keywords with thread safety"""
    from core.knowledge_graph import _graph_lock
    ...
    with _graph_lock:
        for concept, info in ocr_keywords.items():
            try:
                ...
                next_review_time = next_review.strftime("%Y-%m-%d %H:%M:%S") if isinstance(next_review, datetime) else str(next_review)
...
def ask_user_permissions():
    ...
    try:
        ...
        allow_webcam = choice in ['y', 'yes', '1']
