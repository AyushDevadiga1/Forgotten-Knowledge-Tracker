...
# ==========================================================
# Part 3: Sessions, Memory Decay, Predictions, Logs & Reminders
# ==========================================================
...
with tabs[3]:
    ...
    if not df_sessions_filtered.empty:
        ...
        fig_tl.update_layout(title="App Sessions Timeline")
...
# -----------------------------
st.markdown("---")
st.markdown("💡 Dashboard powered by FKT Tracker — Visualizing sessions, memory decay, knowledge graph, and reminders in real-time.")

# ==========================================================
# Part 4: Responsive Stylish Theme + Sidebar Enhancements
# ==========================================================
...
st.markdown(
    ...
    .css-1d391kg {{padding:1rem;}}  /* Adjust main content spacing */
    .css-1v3fvcr {{color:{text_color};}} /* Streamlit text color fix */
    </style>
    """, unsafe_allow_html=True
)

# -----------------------------
# 2️⃣ Sidebar Slider for Tab Navigation
# -----------------------------
st.sidebar.markdown("---")
st.sidebar.subheader("Navigate Tabs")
tab_names = [
    "Overview", "Knowledge Graph", "3D Graph", "Sessions",
    "Memory Decay", "Predicted Forgetting", "Multi-Modal Logs", "Upcoming Reminders"
]
selected_tab = st.sidebar.slider(
    "Select Tab",
    0,
    len(tab_names)-1,
    0,
    step=1,
    format="%d - " + tab_names[0]
)
# Update tab labels dynamically for slider
tabs = st.tabs(tab_names)
active_tab = tabs[selected_tab]

# -----------------------------
# 3️⃣ Sidebar Quick Metrics
# -----------------------------
st.sidebar.markdown("---")
...
upcoming_count = len(df_metrics[df_metrics["next_review_time"] > datetime.now()])

st.sidebar.metric("Total Hours Learned", f"{total_hours:.2f} h")
st.sidebar.metric("Avg Session Length", f"{avg_session:.1f} min")
st.sidebar.metric("Sessions Count", f"{num_sessions}")
st.sidebar.metric("Upcoming Reviews", f"{upcoming_count}")

# -----------------------------
# 4️⃣ Hover & Zoom Styling for Plots
# -----------------------------
plotly_template = "plotly_dark" if theme_choice == "Dark" else "plotly_white"

# Example: Use template for 3D graph
# fig_3d.update_layout(template=plotly_template)

# -----------------------------
# 5️⃣ Footer / Credits Styling
# -----------------------------
st.markdown(
    f"""
    <style>
    ...
    <div class="footer">
        🔮 FKT Tracker Dashboard | Stylish Responsive Theme | © {datetime.now().year}
    </div>
    """, unsafe_allow_html=True
)
