# ==========================================================
# dashboard/dashboard.py (Upgraded IEEE-Grade)
# ==========================================================
import sys
import os
import sqlite3
...
# -----------------------------
st_autorefresh(interval=50000, key="dashboard_refresh")
...
with tabs[0]:
    ...
    if G.nodes:
        mem_table = [{"Concept": n, 
                      "Memory Score": round(G.nodes[n].get("memory_score",0.3),2),
                      "Next Review": G.nodes[n].get("next_review_time","N/A"),
                      "Intent Conf": round(G.nodes[n].get("intent_conf",1.0),2)} 
                     for n in G.nodes]
...
# -----------------------------
# Knowledge Graph Tab (Interactive)
# -----------------------------
with tabs[1]:
    ...
    if G.nodes:
        ...
        for node in G.nodes:
            ...
            node_y.append(y)
            labels.append(node)
            mem_scores.append(G.nodes[node].get("memory_score",0.3))
        for edge in G.edges:
            x0, y0 = pos[edge[0]]
            x1, y1 = pos[edge[1]]
            edge_x += [x0, x1, None]
        ...
        node_trace = go.Scatter(x=node_x, y=node_y,
                                mode='markers+text',
                                text=labels,
                                hovertext=[f"{n}<br>Memory: {G.nodes[n].get('memory_score',0.3):.2f}<br>Next: {G.nodes[n].get('next_review_time','N/A')}" for n in G.nodes],
                                marker=dict(size=[node_min_size + s*(node_max_size-node_min_size) for s in mem_scores],
                                            color=mem_scores, colorscale='Viridis', showscale=True),
...
with tabs[2]:
    ...
    if not df_sessions_filtered.empty:
        fig_tl = px.timeline(df_sessions_filtered, x_start="start_ts", x_end="end_ts", y="app_name", color="app_name")
        fig_tl.update_yaxes(autorange="reversed")
        st.plotly_chart(fig_tl, use_container_width=True)
        heat_df = df_sessions_filtered.groupby(["app_name", df_sessions_filtered["start_ts"].dt.date])["duration_min"].sum().reset_index()
...
with tabs[3]:
    ...
    if not df_decay.empty:
        df_decay["timestamp"] = pd.to_datetime(df_decay["last_seen_ts"], errors="coerce")
        df_decay["memory_score"] = pd.to_numeric(df_decay["predicted_recall"], errors="coerce")
        fig_decay = px.line(df_decay, x="timestamp", y="memory_score", color="keyword", markers=True,
...
# -----------------------------
# Predicted Forgetting (Multiple Concepts)
# -----------------------------
with tabs[4]:
    st.subheader("📈 Predicted Forgetting Overlay")
    selected_pred = st.multiselect("Choose concepts", concepts, default=concepts[:3])
    lambda_val = 0.1
    hours = 24
    if selected_pred:
        fig_pred = go.Figure()
        for concept in selected_pred:
            df_last = df_decay[df_decay["keyword"]==concept]
        ...
        fig_pred.update_layout(yaxis=dict(range=[0,1]), xaxis_title="Time", yaxis_title="Memory Score",
                               title="Predicted Forgetting Curves")
        st.plotly_chart(fig_pred, use_container_width=True)

# -----------------------------
# Multi-Modal Logs Tab (Filterable)
# -----------------------------
with tabs[5]:
    st.subheader("🎤 Multi-Modal Logs")
    if not df_logs.empty:
        app_filter = st.multiselect("Filter by App", df_logs["window_title"].unique())
        intent_filter = st.multiselect("Filter by Intent", df_logs["intent_label"].unique())
        attention_range = st.slider("Attention Range", 0, int(df_logs["attention_score"].max()), (0,int(df_logs["attention_score"].max())))
        df_filtered = df_logs.copy()
        if app_filter: df_filtered = df_filtered[df_filtered["window_title"].isin(app_filter)]
        if intent_filter: df_filtered = df_filtered[df_filtered["intent_label"].isin(intent_filter)]
        df_filtered = df_filtered[(df_filtered["attention_score"]>=attention_range[0]) & (df_filtered["attention_score"]<=attention_range[1])]
        st.dataframe(df_filtered.head(200))
    else:
        st.info("No multi-modal logs available.")

# -----------------------------
# Upcoming Reminders Tab (Highlight Urgent)
# -----------------------------
with tabs[6]:
    ...
    if not upcoming.empty:
        df_display = upcoming.copy()
        df_display["urgent"] = df_display["memory_score"] < 0.5
        st.dataframe(df_display[["concept","next_review_time","memory_score","urgent"]])
    else:
