"""
FKT Dashboard — Fixed & Hardened Streamlit App
- Defensive parsing for DB tables
- Robust date filtering (handles single date or tuple)
- Safer 3D KG rendering (handles missing attributes)
...
Save as: dashboard/fkt_dashboard.py (or open in canvas)
"""
...
# core.knowledge_graph functions may raise — import lazily later where used
try:
    from core.knowledge_graph import get_graph, sync_db_to_graph
except Exception:
    # if import fails we'll handle later by creating an empty Graph
    get_graph = None
...
def load_cleaned_data():
    """Load data from sqlite DB and return cleaned DataFrames.
    Returns: (df_sessions, df_logs, df_decay, df_metrics)
    """
    ...
    try:
        ...
        if not df_sessions.empty:
            ...
            if "start_ts" in df_sessions.columns and "end_ts" in df_sessions.columns:
                ...
                df_sessions["duration_min"] = 0.0
        else:
            # create a consistent empty df with expected columns
        ...
        if not df_decay.empty:
            ...
            # normalize predicted_recall if present
            if "predicted_recall" in df_decay.columns:
                ...
                df_decay["predicted_recall"] = df_decay["predicted_recall"].clip(0, 1)
...
def parse_ocr_keywords_field(val):
    """Safe parse for ocr_keywords stored as JSON string, dict string or comma-separated string.
    Returns a dict mapping keyword->meta where meta is at least {'score': float}.
    """
    if val is None:
        return {}
    ...
    if isinstance(val, (list, tuple)):
        return {str(x): {"score": 0.5} for x in val}
    if not isinstance(val, str):
        # sometimes pandas types can be 'object' with non-str; coerce
        try:
            s = str(val)
        except Exception:
            return {}
    ...
    try:
        ...
        if isinstance(parsed, list):
            return {str(k): {"score": 0.5} for k in parsed}
    except Exception:
        pass

    # try fallback replace single quotes
    try:
    ...
    # comma / newline / semicolon separated fallback
...
# Apply date filter to sessions if possible
# date_range may be a single date, or a list/tuple of two dates
...
if not df_sessions.empty and "start_ts" in df_sessions.columns:
    try:
        ...
        # fallback: if start_ts isn't datetime, coerce and retry
        df_sessions["start_ts"] = pd.to_datetime(df_sessions.get("start_ts"), errors="coerce")
        df_sessions = df_sessions[(df_sessions["start_ts"].dt.date >= start_date) &
                                  (df_sessions["start_ts"].dt.date <= end_date)]

# Sync & load KG (defensive)
G = nx.Graph()
if sync_db_to_graph is not None and get_graph is not None:
    ...
    # try to read a simple serialized graph file if present
    try:
...
def card_html(title: str, value: str, subtitle: str = ""):
...
with tabs[0]:
    ...
    if not df_sessions.empty:
        ...
        if "start_ts" in recent.columns:
            ...
            try:
                ...
                c1.line_chart(recent.set_index("start_ts")["duration_min"].fillna(0))
    ...
    if len(G.nodes):
        ...
        for n in G.nodes:
            try:
                mem_list.append({"Concept": n, "Memory Score": float(G.nodes[n].get("memory_score", 0.3))})
            except Exception:
                mem_list.append({"Concept": n, "Memory Score": 0.3})
...
with tabs[3]:
    ...
    if df_decay.empty:
        ...
        if "timestamp" in df_decay2.columns:
            df_decay2["timestamp"] = pd.to_datetime(df_decay2["timestamp"], errors="coerce")
        # ensure predicted_recall exists
        if "predicted_recall" not in df_decay2.columns:
            df_decay2["predicted_recall"] = 0.0
        df_decay2["predicted_recall"] = pd.to_numeric(df_decay2["predicted_recall"], errors="coerce").fillna(0).clip(0,1)
...
with tabs[4]:
    ...
    if len(concepts) == 0:
        ...
        if not df_decay.empty and "concept" in df_decay.columns:
            df_last = df_decay[df_decay["concept"] == str(selected_pred)].sort_values(by=["last_seen_ts"], ascending=True)
        ...
        if not df_last.empty and "predicted_recall" in df_last.columns:
            try:
                ...
                last_score = float(df_last["predicted_recall"].dropna().iloc[-1]) if not df_last["predicted_recall"].dropna().empty else 0.5

        times = np.linspace(0, hours, 100)
        predicted_scores = last_score * np.exp(-lambda_val * times)
        fig_pred = go.Figure()
        fig_pred.add_trace(go.Scatter(x=[datetime.now() + timedelta(hours=float(t)) for t in times], y=predicted_scores, mode="lines", name="Predicted"))
