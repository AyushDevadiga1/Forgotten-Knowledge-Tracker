...
with tabs[3]:
    ...
    def fetch_decay(concept=None):
        try:
            ...
            st.error(f"Failed to fetch memory decay data: {e}")
            return pd.DataFrame(columns=["concept", "timestamp", "memory_score"])

    df_decay_all = fetch_decay()
    if not df_decay_all.empty:
        fig_decay = px.line(
            df_decay_all, x="timestamp", y="memory_score", color="concept",
            markers=True, title="Memory Decay Per Concept",
    ...
    concepts = list(G.nodes) if G and G.nodes else []
...
with tabs[4]:
    ...
    if concepts:
        ...
        st.info("No concepts available for predicted forgetting.")
...
with tabs[5]:
    ...
    if not df_logs.empty:
        try:
            ...
            st.error(f"Failed to display logs: {e}")
    else:
