...
def load_cleaned_data():
    ...
    # Upcoming Reminders (from metrics table)
    df_metrics = pd.read_sql("SELECT concept, next_review_time FROM metrics ORDER BY next_review_time ASC", conn)
...
st.markdown("Visualize your learning sessions, memory scores, knowledge graph, and reminders.")
...
# Tabs
# -----------------------------
tabs = st.tabs(["Overview", "Knowledge Graph", "Sessions", "App Usage", "Memory Decay", "Predicted Forgetting", "Upcoming Reminders", "Multi-Modal Logs"])
...
with tabs[3]:
    ...
    if not df_sessions.empty:
        app_usage = df_sessions.groupby("app_name")["duration_min"].sum().reset_index()
        app_usage = app_usage.sort_values("duration_min", ascending=False)
        fig_app = px.bar(app_usage, x="app_name", y="duration_min", color="duration_min",
                         labels={"app_name":"App", "duration_min":"Total Duration (min)"},
                         title="Total Time Spent Per App")
        st.plotly_chart(fig_app, use_container_width=True)

        fig_line = px.line(df_sessions.groupby([df_sessions["start_ts"].dt.date, "app_name"])["duration_min"].sum().reset_index(),
                           x="start_ts", y="duration_min", color="app_name", markers=True,
                           labels={"start_ts":"Date","duration_min":"Duration (min)"},
                           title="Daily App Usage Line Graph")
...
# -----------------------------
with tabs[6]:
    st.subheader("⏰ Upcoming Review Reminders")
...
# -----------------------------
# Multi-Modal Logs Tab (Fix bytes issue)
# -----------------------------
with tabs[7]:
    ...
    if not df_logs.empty:
        ...
        for col in df_display.columns:
            if df_display[col].dtype == object:
                df_display[col] = df_display[col].apply(lambda x: x.hex() if isinstance(x, (bytes, bytearray)) else x)
