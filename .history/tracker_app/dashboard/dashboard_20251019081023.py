# ==========================================================
# dashboard/fkt_dashboard.py | FKT Dashboard v6
# ==========================================================
...
.metric-card {background: linear-gradient(160deg, rgba(22,27,34,0.85), rgba(10,12,16,0.6));
border-radius: 14px; padding: 14px; margin: 6px; box-shadow: 0 6px 22px rgba(0,0,0,0.5);
text-align: center;}
...
st.markdown("Interactive dashboard for sessions, memory scores, knowledge graph, performance insights.")
...
def load_cleaned_data():
    ...
    try:
        ...
        try:
            ...
            df_metrics = pd.DataFrame(columns=["concept","next_review_time","memory_score"])
...
col1.markdown(card_html("Total Hours", f"{df_sessions['duration_min'].sum()/60:.2f} h", "Across sessions"), unsafe_allow_html=True)
col2.markdown(card_html("Avg Session", f"{df_sessions['duration_min'].mean():.1f} min", "Mean session duration"), unsafe_allow_html=True)
col3.markdown(card_html("Sessions", f"{len(df_sessions)}", "Total sessions loaded"), unsafe_allow_html=True)
col4.markdown(card_html("Unique Concepts", f"{len(concepts)}", "Nodes in knowledge graph"), unsafe_allow_html=True)
st.markdown("---")
...
# ----------------------------- Dynamic Intents → Tabs -----------------------------
unique_intents = df_sessions['intent_label'].value_counts().index.tolist()
intent_to_tabs = {}
for intent in unique_intents:
    # Assign tabs heuristically (first 2 tabs per intent)
    intent_to_tabs[intent] = tabs_list[1:4]  # example: KG, Sessions, Memory
...
with tabs[0]:
    ...
    if not df_sessions.empty:
        ...
        st.metric("Concepts tracked", len(concepts))
    else:
        st.info("No session data available.")

# ----------------------------- Knowledge Graph (3D) Tab -----------------------------
with tabs[1]:
    st.header("Hierarchical 3D Knowledge Graph")
    # Create 3D nodes dynamically
    nodes, edges, labels = [], [], []
    pos = {}
    z_intent, z_tab = 10, 0
    x_spacing, y_spacing = 10, 5
    for i,intent in enumerate(unique_intents):
        x_intent = i*x_spacing; y_intent = 0
        pos[intent] = (x_intent,y_intent,z_intent)
        nodes.append(intent); labels.append(intent)
        for j,tab_name in enumerate(intent_to_tabs[intent]):
            x_tab = x_intent + (j-(len(intent_to_tabs[intent])-1)/2)*y_spacing
            y_tab = 0
            pos[tab_name] = (x_tab,y_tab,z_tab)
            nodes.append(tab_name); labels.append(tab_name)
            edges.append((intent,tab_name))

    # Edge coordinates
    edge_x,edge_y,edge_z=[],[],[]
    for e in edges:
        x0,y0,z0 = pos[e[0]]; x1,y1,z1 = pos[e[1]]
        edge_x += [x0,x1,None]; edge_y += [y0,y1,None]; edge_z += [z0,z1,None]

    x_nodes = [pos[n][0] for n in nodes]; y_nodes = [pos[n][1] for n in nodes]; z_nodes = [pos[n][2] for n in nodes]
    node_colors = ['red' if n in unique_intents else 'orange' for n in nodes]

    fig_3d = go.Figure()
    fig_3d.add_trace(go.Scatter3d(x=edge_x,y=edge_y,z=edge_z,mode='lines',
                                  line=dict(color='black',width=3)))
    fig_3d.add_trace(go.Scatter3d(x=x_nodes,y=y_nodes,z=z_nodes,mode='markers+text',
                                  marker=dict(size=10,color=node_colors),
                                  text=labels,textposition='top center'))
    fig_3d.update_layout(scene=dict(xaxis=dict(showbackground=False,visible=False),
                                    yaxis=dict(showbackground=False,visible=False),
                                    zaxis=dict(showbackground=False,visible=False)),
                         margin=dict(l=0,r=0,t=30,b=0),showlegend=False,template="plotly_dark")
...
# ----------------------------- Other tabs -----------------------------
# You can copy the previous code for tabs[3:] (Memory Decay, Predicted Forgetting, Multi-Modal, OCR, Audio/Intent, Visual, Performance, System Health)
# Keep defensive parsing and plots as in your previous version

st.caption("FKT Dashboard v6 — Dynamic intents → tabs 3D KG, rich visuals, KPIs, system diagnostics.")
