...
def load_cleaned_data():
    ...
    # Only keep rows where timestamps exist and are valid
    df_sessions = df_sessions[pd.to_datetime(df_sessions["start_ts"], errors='coerce').notna()]
    df_sessions = df_sessions[pd.to_datetime(df_sessions["end_ts"], errors='coerce').notna()]

    df_sessions["start_ts"] = pd.to_datetime(df_sessions["start_ts"])
    df_sessions["end_ts"] = pd.to_datetime(df_sessions["end_ts"])

    # Keep only positive durations
    df_sessions = df_sessions[df_sessions["end_ts"] > df_sessions["start_ts"]]
    df_sessions["duration_min"] = (df_sessions["end_ts"] - df_sessions["start_ts"]).dt.total_seconds() / 60

    # Keep apps as string
    df_sessions = df_sessions[df_sessions["app_name"].notna()]
    df_sessions["app_name"] = df_sessions["app_name"].astype(str)
    ...
    df_logs = df_logs[df_logs["timestamp"].notna()]

    # Convert bytes columns to string if required
    for col in df_logs.columns:
        if df_logs[col].dtype == object:
            df_logs = df_logs[df_logs[col].apply(lambda x: isinstance(x, (bytes, bytearray)) == False)]
    ...
    df_decay = df_decay[df_decay["last_seen_ts"].notna()]
    df_decay["predicted_recall"] = pd.to_numeric(df_decay["predicted_recall"], errors='coerce')
    df_decay = df_decay[df_decay["predicted_recall"].notna()]
    ...
    df_metrics = df_metrics[df_metrics["next_review_time"].notna()]
    df_metrics["memory_score"] = pd.to_numeric(df_metrics["memory_score"], errors='coerce')
    df_metrics = df_metrics[df_metrics["memory_score"].notna()]
