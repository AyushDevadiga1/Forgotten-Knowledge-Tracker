...
# -----------------------------
# -----------------------------
# 3D Knowledge Graph Tab (Hierarchical)
# -----------------------------
with tabs[2]:
    st.subheader("🧩 3D Knowledge Graph: Intent → Tabs (Hierarchical)")

    # Define intents and their corresponding tab/topic nodes
    intents = ["Intent A", "Intent B", "Intent C"]
    intent_to_tabs = {
        "Intent A": ["Overview", "Memory Decay", "Predicted Forgetting"],
        "Intent B": ["Sessions", "Multi-Modal Logs"],
        "Intent C": ["Upcoming Reminders", "Knowledge Graph"]
    }
    ...
    z_intent = 10  # z-level for intent nodes
    z_tab = 0      # z-level for tab/topic nodes
    x_spacing = 10  # horizontal spacing between intents
    y_spacing = 5   # horizontal spacing for tabs beneath intent

    # Position dictionaries for Plotly
    pos = {}

    for i, intent in enumerate(intents):
        x_intent = i * x_spacing
        y_intent = 0
        pos[intent] = (x_intent, y_intent, z_intent)
        nodes.append(intent)
        node_labels.append(intent)
        # Add tab nodes below
        tabs_list = intent_to_tabs[intent]
        for j, tab in enumerate(tabs_list):
            x_tab = x_intent + (j - (len(tabs_list)-1)/2) * y_spacing
            y_tab = 0
            pos[tab] = (x_tab, y_tab, z_tab)
            nodes.append(tab)
            node_labels.append(tab)
            edges.append((intent, tab))
    ...
    # Colors: intents one color, tabs another
    node_colors = ['red' if n in intents else 'orange' for n in nodes]
    ...
    fig_3d.update_layout(
        ...
        height=700,
        title="Hierarchical 3D Intent → Tabs Graph"
    )
