...
with tabs[0]:
    ...
    # Local conversion & duration handling (isolated to this tab)
    df_sessions_local = df_sessions.copy()
    df_sessions_local["start_ts"] = pd.to_datetime(df_sessions_local["start_ts"], errors='coerce', utc=True)
    df_sessions_local["end_ts"] = pd.to_datetime(df_sessions_local["end_ts"], errors='coerce', utc=True)

    # Fallback if end_ts invalid or missing
    mask_invalid = (df_sessions_local["end_ts"].isna()) | (df_sessions_local["end_ts"] < df_sessions_local["start_ts"])
    df_sessions_local.loc[mask_invalid, "end_ts"] = (
        df_sessions_local.loc[mask_invalid, "start_ts"] + pd.Timedelta(minutes=5)
    )

    # Compute duration in minutes
    df_sessions_local["duration_min"] = (
        (df_sessions_local["end_ts"] - df_sessions_local["start_ts"]).dt.total_seconds() / 60
    ).fillna(0)

    df_sessions_filtered = df_sessions_local.dropna(subset=["start_ts", "end_ts"])
    ...
    if not df_sessions_filtered.empty:
        daily_hours = (
            df_sessions_filtered.groupby(df_sessions_filtered["start_ts"].dt.date)["duration_min"].sum() / 60
        )
