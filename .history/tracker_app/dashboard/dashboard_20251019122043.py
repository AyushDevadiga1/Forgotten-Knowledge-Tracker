...
# -----------------------------
with tabs[2]:
    st.subheader("🧩 3D Knowledge Graph by Final Intent")
    
    # Get unique intents
    intents = df_sessions['intent_label'].dropna().unique()
    if len(intents) == 0:
        st.info("No intents found in DB.")
    else:
        selected_intent = st.selectbox("Select Final Intent", intents)
        
        # Filter sessions by selected intent
        df_intent = df_sessions[df_sessions['intent_label'] == selected_intent]
        
        edges = []
        nodes = set()
        for _, row in df_intent.iterrows():
            node_name = f"{row['audio_label']}_{row['id']}"
            edges.append((selected_intent, node_name))
            nodes.add(selected_intent)
            nodes.add(node_name)
        ...
        if edges:
            ...
            # nodes with non-overlapping labels
            fig_3d.add_trace(go.Scatter3d(
            ...
            fig_3d.update_layout(
                title=f"3D Graph for Intent: {selected_intent}",
                scene=dict(
                    xaxis=dict(title='X'),
                    yaxis=dict(title='Y'),
                    zaxis=dict(title='Z')
            ...
            st.info("No keywords for this intent.")
...
with tabs[4]:
    ...
    if not df_decay.empty:
        # Aggregate latest decay for each keyword
        latest_decay = df_decay.groupby('keyword').last().reset_index()
        
        # 3D scatter over time for clarity
        fig_mem = go.Figure()
        for i, row in latest_decay.iterrows():
            fig_mem.add_trace(go.Scatter3d(
                x=[row['last_seen_ts']],
                y=[i],
                z=[row['predicted_recall']],
                mode='markers+text',
                text=[row['keyword']],
                textposition='top center',
                marker=dict(size=6, color=row['predicted_recall'], colorscale='Viridis', opacity=0.8),
                name=row['keyword']
            ))
        
        fig_mem.update_layout(
            title="Memory Decay per Keyword",
            scene=dict(
                xaxis=dict(title='Last Seen Time'),
                yaxis=dict(title='Keyword Index'),
...
# -----------------------------
with tabs[4]:
    st.subheader("📉 Memory Decay Radar")
    if not df_decay.empty:
