...
with tabs[2]:
    ...
    if G3d.nodes:
        ...
        memory_scores = [G3d.nodes[n].get('memory_score',0.3) for n in G3d.nodes()]
        ...
        fig_3d.add_trace(go.Scatter3d(
            ...
            mode='markers+text',
            marker=dict(
                size=[5+15*s for s in memory_scores],
                color=memory_scores,
                colorscale='Viridis',
                opacity=0.8,
                line=dict(width=1, color='black')
            ),
            text=[f"{n}<br>Memory: {G3d.nodes[n].get('memory_score',0):.2f}" for n in G3d.nodes()],
            hoverinfo='text'
        ))
