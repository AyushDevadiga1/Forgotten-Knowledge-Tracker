...
with tabs[1]:
    ...
    if G.nodes:
        # Just compute layout directly; no caching
        pos = nx.spring_layout(G, seed=42, k=0.8)

        # Limit nodes if too large
        max_nodes = 100
        if len(G) > max_nodes:
            nodes_keep = list(G.nodes)[:max_nodes]
            G = G.subgraph(nodes_keep)
            pos = {n: pos[n] for n in nodes_keep}
...
with tabs[2]:
    ...
    @st.cache_data
    def build_3d_graph(DB_PATH, max_nodes=50):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
    ...
    edges, nodes = build_3d_graph(DB_PATH)
...
with tabs[3]:
    ...
    if not df_sessions.empty:
        df_sess = df_sessions.copy().head(200)  # limit rows
        ...
        heat_pivot = heat_df.pivot(index="app_name", columns="start_ts", values="duration").fillna(0).iloc[:20,:30]
...
with tabs[4]:
    ...
    if not df_decay.empty:
        df_decay_plot = df_decay.copy().head(200)  # limit rows for plotting
        fig_decay = px.line(df_decay_plot, x="last_seen_ts", y="predicted_recall", color="keyword", markers=True,
