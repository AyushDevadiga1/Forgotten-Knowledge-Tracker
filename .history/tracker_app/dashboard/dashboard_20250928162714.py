...
from core.knowledge_graph import get_graph, sync_db_to_graph, add_concepts
from core.db_module import DB_PATH, decrypt_data
...
# -----------------------------
# Optional: Live session_data display
# -----------------------------
def display_session_data(session_data):
    st.subheader("🔹 Live Session Data")
    st.write("Window:", session_data.get("window_title", "N/A"))
    st.write("Interaction Rate:", session_data.get("interaction_rate", 0))
    st.write("OCR Keywords:", session_data.get("ocr_keywords", []))
    st.write("Audio Label:", session_data.get("audio_label", "silence"))
    st.write("Attention Score:", session_data.get("attention_score", 0))
    st.write("Intent:", session_data.get("intent_label", ""), f"({session_data.get('intent_confidence', 0.0):.2f})")

# If using live session_data from tracker, call display_session_data(session_data)
...
try:
    ...
    c = conn.cursor()
    c.execute("SELECT start_ts, end_ts, app_name, window_title, interaction_rate FROM sessions ORDER BY start_ts DESC LIMIT 50")
    rows = c.fetchall()
    conn.close()
    ...
    if len(rows) == 0:
        ...
        # Decrypt if needed
        rows = [(start, end, decrypt_data(app), decrypt_data(win), rate) for start, end, app, win, rate in rows]

        session_df = pd.DataFrame(rows, columns=["Start", "End", "App", "Window", "Interaction"])
        ...
        fig2 = px.timeline(
            ...
            y="App",
            color="App",
            hover_data=["Start", "End", "Window", "Interaction"],
