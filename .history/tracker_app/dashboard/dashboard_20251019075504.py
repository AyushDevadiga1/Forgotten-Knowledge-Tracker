fix these version : # dashboard/fkt_dashboard.py
"""
FKT Dashboard — Upgraded with Rich Visuals
- Dark theme, KPI cards, 3D interactive KG, Performance Insights, System Health
- All prior tabs preserved and optimized (caching, defensive parsing)
"""
...
# Inject custom CSS for cards and theme refinements
st.markdown(
    """
    <style>
...
total_hours = (df_sessions["duration_min"].sum() / 60) if not df_sessions.empty else 0.0
avg_session = df_sessions["duration_min"].mean() if not df_sessions.empty else 0.0
...
with tabs[1]:
    ...
    if len(G.nodes) == 0:
        ...
        try:
            ...
            # project to 3D by adding small z perturbation based on centrality
            centrality = nx.degree_centrality(G)
            nodes = list(G.nodes)
            x, y, z, text, size, color = [], [], [], [], [], []
            mem_scores = [G.nodes[n].get("memory_score", 0.3) for n in nodes]
            norm = mcolors.Normalize(vmin=0, vmax=1)
            cmap = cm.plasma

            for i, n in enumerate(nodes):
                px_, py_ = pos2d[n]
                pz_ = centrality.get(n, 0) * 0.8  # small z
                x.append(px_)
                y.append(py_)
                z.append(pz_)
                ms = float(G.nodes[n].get("memory_score", 0.3))
                text.append(f"{n}<br>Memory: {ms:.2f}<br>Count: {G.nodes[n].get('count',0)}")
            ...
            for e in G.edges():
                n1, n2 = e
                try:
                    edge_x += [pos2d[n1][0], pos2d[n2][0], None]
                    edge_y += [pos2d[n1][1], pos2d[n2][1], None]
