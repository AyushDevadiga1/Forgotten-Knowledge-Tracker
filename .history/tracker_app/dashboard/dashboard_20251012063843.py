...
with tabs[3]:
    ...
    if not df_decay_all.empty:
        ...
        df_decay_all["memory_score"] = pd.to_numeric(df_decay_all["memory_score"], errors="coerce")
    ...
    if selected:
        ...
        for c in selected:
            ...
            if not df_c.empty:
                ...
                fig_ind.add_trace(go.Scatter(x=df_c["timestamp"], y=df_c["memory_score"],
                                             mode="lines+markers", name=c))
        fig_ind.update_layout(title="Individual Memory Decay", yaxis=dict(range=[0, 1]),
