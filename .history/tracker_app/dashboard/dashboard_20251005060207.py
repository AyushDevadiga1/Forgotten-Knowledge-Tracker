...
def load_cleaned_data():
    ...
    # Remove invalid sessions where start or end is missing or negative duration
    ...
    # Convert bytes columns to hex for Streamlit
    for col in df_logs.columns:
        if df_logs[col].dtype == object:
            df_logs[col] = df_logs[col].apply(lambda x: x.hex() if isinstance(x, (bytes, bytearray)) else x)
    ...
    df_decay["predicted_recall"] = pd.to_numeric(df_decay["predicted_recall"], errors='coerce').fillna(0.0)

    # Metrics / Reminders
    df_metrics = pd.read_sql("SELECT concept, next_review_time, memory_score FROM metrics", conn)
    df_metrics["next_review_time"] = pd.to_datetime(df_metrics["next_review_time"], errors='coerce')
    df_metrics["memory_score"] = pd.to_numeric(df_metrics["memory_score"], errors='coerce').fillna(0.0)
...
# -----------------------------
# Fixed Avg Session Calculation
# -----------------------------
if not df_sessions.empty:
    avg_session = df_sessions["duration_min"].mean()
    total_hours = df_sessions["duration_min"].sum() / 60
else:
    avg_session = 0
    total_hours = 0
