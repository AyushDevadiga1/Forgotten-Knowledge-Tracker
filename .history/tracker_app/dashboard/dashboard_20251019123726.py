...
with tabs[0]:
    ...
    if df_sessions_filtered.empty:
        st.info("No session data found.")
    else:
        # Calculate key metrics
        total_hours = df_sessions_filtered["duration_min"].sum() / 60
        avg_session = df_sessions_filtered["duration_min"].mean()
        num_sessions = len(df_sessions_filtered)
        total_idle = df_sessions_filtered[df_sessions_filtered['interaction_count'] == 0]["duration_min"].sum()
        
        # Top apps
        top_apps = df_sessions_filtered.groupby("app_name")["duration_min"].sum().sort_values(ascending=False).head(10)
        
        # Top keywords (from audio labels)
        top_keywords = df_sessions_filtered['audio_label'].value_counts().head(10)
        ...
        # Top apps usage (bar chart)
        st.markdown("### Top 10 Apps Used")
        fig_apps = px.bar(
            top_apps,
            x=top_apps.index,
            y=top_apps.values,
            labels={'x':'App', 'y':'Hours Studied'},
        ...
        st.markdown("### Most Frequent Keywords / Topics")
        fig_keywords = px.bar(
            top_keywords,
        ...
        # Idle vs Active time (pie chart)
        active_time = total_hours*60 - total_idle
        st.markdown("### Active vs Idle Time")
        fig_idle = px.pie(
            names=['Active', 'Idle'],
            values=[active_time, total_idle],
            color=['Active','Idle'],
            color_discrete_map={'Active':'green', 'Idle':'red'}
        )
