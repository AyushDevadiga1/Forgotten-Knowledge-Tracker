    ...
    try:
        ...
        for col in ["start_ts", "end_ts"]:
            if col in df_sessions.columns:
                ...
                df_sessions[col] = pd.NaT
        df_sessions["app_name"] = df_sessions.get("app_name", "Unknown App")
        ...
        try:
            ...
            df_metrics["next_review_time"] = pd.to_datetime(df_metrics.get("next_review_time"), errors='coerce')
        except Exception:
...
if not df_sessions.empty and "start_ts" in df_sessions.columns and "end_ts" in df_sessions.columns:
...
# -----------------------------
tabs = st.tabs([
    "Overview 📊", "Knowledge Graph 🕸️", "Sessions ⏱️",
...
def fetch_decay(concept=None):
    ...
    try:
        ...
        # Convert all concepts to string for safety
