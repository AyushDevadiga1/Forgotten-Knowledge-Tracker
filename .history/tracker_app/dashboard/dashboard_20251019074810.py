...
# -----------------------------
# -----------------------------
# 3D Knowledge Graph Tab (Intents → Tabs)
# -----------------------------
with tabs[2]:
    st.subheader("🧩 3D Knowledge Graph (Intent → Tabs)")
    ...
    # Build edges (Intent → Tabs)
    edges = []
    nodes = set()
    for intent in intents:
        cursor.execute("SELECT DISTINCT audio_label || '_' || rowid FROM sessions WHERE intent_label=?", (intent,))
        tabs_for_intent = [row[0] for row in cursor.fetchall()[:5]]  # limit 5 tabs per intent for clarity
        for tab in tabs_for_intent:
    ...
    if edges:
        ...
        pos_3d = nx.spring_layout(G3d, dim=3, seed=42, k=1)
        ...
        # Separate intents and tabs for styling
        intents_set = set(intents)
        node_colors = ['orange' if n in intents_set else 'lightblue' for n in labels]
        node_sizes = [15 if n in intents_set else 8 for n in labels]
        ...
        # Add labels offset slightly from nodes
        label_offsets = 0.03  # small offset
        for i, label in enumerate(labels):
            x, y, z = x_nodes[i], y_nodes[i], z_nodes[i]
            offset_x = x + label_offsets
            offset_y = y + label_offsets
            offset_z = z + label_offsets
            fig_3d.add_trace(go.Scatter3d(
        ...
        fig_3d.update_layout(
            ...
            height=700,
            title="3D Open-Space Graph: Intent → Tabs",
            margin=dict(l=0, r=0, t=40, b=0)
        )
        st.plotly_chart(fig_3d, use_container_width=True)
    else:
        st.info("No intents or tabs found in DB.")
