...
from datetime import datetime, timedelta
from core.tracker import maybe_notify, compute_memory_score, schedule_next_review
...
class TestTrackerEdgeCases(unittest.TestCase):
    ...
    def test_memory_notify_cooldown(self, mock_notify):
        # Create a minimal graph for the concept
        G = nx.DiGraph()
        G.add_node("concept_test", last_reminded_time=(datetime.now() - timedelta(hours=1)).isoformat())
        ...
        # Call maybe_notify with a proper graph
        maybe_notify("concept_test", score, G, use_attention=False)

        # Check that graph updated last_reminded_time
        self.assertIn("last_reminded_time", G.nodes["concept_test"])
