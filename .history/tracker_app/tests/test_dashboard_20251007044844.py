...
def test_sessions_duration_positive(data):
    df_valid = df_sessions[df_sessions["end_ts"] > df_sessions["start_ts"]]
    durations = (df_valid["end_ts"] - df_valid["start_ts"]).dt.total_seconds() / 60
    assert (durations > 0).all() or df_valid.empty
