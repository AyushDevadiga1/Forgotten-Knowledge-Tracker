...
def log_session(window_title, interaction_rate):
    ...
    # Ensure interaction_rate is a native int
...
def log_multi_modal(window, ocr_keywords, audio_label, attention_score, interaction_rate, intent_label, intent_confidence):
    """
    Log multi-modal data safely, ensuring native Python types and correct OCR structure.
    """
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
        ...
        try:
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                for concept, info in latest_ocr.items():
                    ...
                    mem_score = float(compute_memory_score(last_review, lambda_val, intent_conf, attention_score, audio_conf))
            ...
            if webcam_counter >= WEBCAM_INTERVAL and USER_ALLOW_WEBCAM:
                try:
                    ...
                    latest_attention = int(num_faces if num_faces is not None else 0)
            ...
            # -------- Intent --------
            intent_data = predict_intent_live(latest_ocr, latest_audio, latest_attention, latest_interaction)
            print(f"Intent -> {intent_data['intent_label']} | Confidence: {intent_data['confidence']:.2f}")

            # -------- Log Multi-Modal Data --------
            log_multi_modal(window, latest_ocr, latest_audio, latest_attention,
                            latest_interaction, intent_data['intent_label'], intent_data['confidence'])
