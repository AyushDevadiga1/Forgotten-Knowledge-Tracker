...
------------------------------
Provides safe creation of session, multi-modal, memory_decay, and metrics tables.
...
def init_metrics_db(db_path: Optional[str] = None) -> None:
    """Track next review times and memory scores per concept"""
    try:
        with get_db_connection(db_path) as conn:
            c = conn.cursor()
            c.execute('''
            ...
            ''')
            # Optionally, populate metrics from memory_decay if empty
            ...
            if c.fetchone()[0] == 0:
                ...
                for concept, recall, last_seen in rows:
                    ...
                    c.execute(
                        'INSERT OR REPLACE INTO metrics (concept, next_review_time, memory_score) VALUES (?, ?, ?)',
...
# -----------------------------
# SELF-TEST / INITIALIZATION SCRIPT
# -----------------------------
