...
def track_loop():
    ...
    # Global dictionary to store keyword occurrences across the session
keyword_occurrences = {}
    ...
    while True:
        try:
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                # OCR keywords now a dict: {keyword: score}
                raw_keywords = ocr_data.get('keywords', [])
                latest_ocr = {kw: score for kw, score in raw_keywords} if raw_keywords else {}
                ocr_counter = 0
                ...
                G = get_graph()
                for concept, kw_score in latest_ocr.items():
                    ...
                    # weight memory by keyword importance
