...
if os.path.exists(intent_clf_path) and os.path.exists(intent_map_path):
    try:
        ...
        logger.exception("Failed to load intent classifier/label map.")
...
def ask_user_permissions() -> None:
    ...
    try:
        ...
        logger.warning("Unable to read user consent input; defaulting to False.")
...
def log_session(window_title: str, interaction_rate: int) -> None:
    ...
    try:
        ...
        conn.commit()
        logger.debug("Logged session: %s (%s)", app_name, interaction_rate)
    except Exception:
...
def log_multi_modal(
    ...
    try:
        ...
        conn.commit()
        logger.debug("Logged multi-modal event for '%s' intent='%s'", window, intent_label)
    except Exception:
...
def predict_intent_live(
    ocr_keywords: Dict[str, Any], audio_label: str, attention_score: Optional[int], interaction_rate: int, use_webcam: bool = False
) -> Dict[str, Any]:
    try:
...
def maybe_notify(concept: str, memory_score: float, graph, use_attention: bool = True) -> None:
    ...
    if memory_score < MEMORY_THRESHOLD and (now - last_reminded >= REMINDER_COOLDOWN):
        ...
        if not use_attention:
            logger.debug("Webcam off or attention ignored; skipping reminder for '%s'.", concept)

# -----------------------------
_latest_ocr: Dict[str, Any] = {}
_latest_attention: Optional[int] = 0

def get_latest_ocr() -> Dict[str, Any]:
    return _latest_ocr

def get_latest_attention() -> Optional[int]:
    return _latest_attention
            ...
            try:
                ...
                if webcam_counter >= WEBCAM_INTERVAL:
                    if USER_ALLOW_WEBCAM:
                        try:
                            ...
                            logger.debug("Faces detected: %s", num_faces)
                        except Exception:
                            logger.exception("Webcam pipeline failed.")
                ...
                for concept, info in _latest_ocr.items():
                    try:
                        ...
                        logger.exception("Error updating memory for concept '%s'", concept)

                # -------- Intent --------
                intent_data = predict_intent_live(
                    _latest_ocr, latest_audio, _latest_attention, latest_interaction, use_webcam=USER_ALLOW_WEBCAM
                )
                ...
                logger.info("Tracker stopped by KeyboardInterrupt.")
                break
            except Exception:
                time.sleep(TRACK_INTERVAL)
                continue
...
if __name__ == "__main__":
    ...
    logger.info("Starting tracker (main).")
    print("Starting tracker...")
    track_loop()
