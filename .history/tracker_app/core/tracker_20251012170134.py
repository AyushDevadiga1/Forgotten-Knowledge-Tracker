...
Includes:
- Memory score computation & reminders
- Knowledge graph updates & co-occurrence edges
- Audio / intent classification (with fallback heuristics)
- Multi-modal logging (OCR, audio, intent, attention, memory)
- Robust error handling & periodic save
"""
...
# -----------------------------
# Globals for latest OCR/attention
# -----------------------------
        ...
        while not stop_event.is_set():
            try:
                ...
                try:
                    ...
                    log_multi_modal(
                        window, _latest_ocr, latest_audio, _latest_attention, latest_interaction,
                        intent_data.get("intent_label", "unknown"), float(intent_data.get("confidence", 0.0)), memory_score
                    )
                except Exception:
                    pass
