...
# ✅ session manager
from session_manager import create_session, update_session
...
def track_loop():
    ...
    while True:
        try:
            ...
            # ✅ update session data
            update_session(session_data, "window_title", window)
            update_session(session_data, "interaction_rate", latest_interaction)
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                # ✅ update session data
                update_session(session_data, "ocr_keywords", latest_ocr)
            ...
            if audio_counter >= AUDIO_INTERVAL:
                ...
                # ✅ update session data
                update_session(session_data, "audio_label", latest_audio)
            ...
            if webcam_counter >= WEBCAM_INTERVAL and USER_ALLOW_WEBCAM:
                try:
                    ...
                    # ✅ update session data
                    update_session(session_data, "attention_score", latest_attention)
            ...
            # ✅ update session data
            update_session(session_data, "intent_label", intent_data['intent_label'])
            update_session(session_data, "intent_confidence", intent_data['confidence'])
