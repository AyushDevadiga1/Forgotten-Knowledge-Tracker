...
def record_audio(duration: int = DURATION) -> np.ndarray:
    ...
    try:
        ...
        logging.error(f"[AudioModule] Audio recording failed: {e}")
...
def extract_features(audio: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        logging.error(f"[AudioModule] Audio feature extraction failed: {e}")
        return np.zeros(14)
...
def classify_audio(audio: np.ndarray) -> Tuple[str, float]:
    ...
    try:
        ...
        logging.error(f"[AudioModule] Audio classification failed: {e}")
        return "unknown", 0.0
...
def audio_pipeline() -> Dict[str, float]:
    ...
    logging.info(f"[AudioModule] Audio classified: {label} (confidence={confidence:.2f})")
