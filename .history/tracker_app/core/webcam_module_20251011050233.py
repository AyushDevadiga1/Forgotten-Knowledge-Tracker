...
# -----------------------------
# -----------------------------
# Robust webcam pipeline
# -----------------------------
def webcam_pipeline_safe(num_frames: int = 10) -> Dict[str, float]:
    """
    Capture webcam frames and compute attention score safely.
    Handles:
      - Missing predictor
      - Invalid frame types
      - Empty frames / no faces
    Returns attentiveness score 0 if any issue occurs.
    """
    ...
    for _ in range(num_frames):
        ...
        if len(frame.shape) == 2:
            ...
            logging.warning(f"[WebcamModule] Unsupported frame shape: {frame.shape}")
            continue
