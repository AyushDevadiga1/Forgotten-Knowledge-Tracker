...
async def ocr_pipeline(roi: Optional[Tuple[int,int,int,int]] = None) -> Dict[str, Any]:
    ...
    -> memory score computation -> knowledge graph update
    """
    ...
    for kw, meta in keywords_filtered.items():
        ...
        last_review_time = now  # could fetch actual last_review from DB for kw
        memory_score = memory_model.compute_memory_score(
            last_review_time=last_review_time,
            lambda_val=0.1,  # default decay, can be adjusted
            intent_conf=1.0,
            attention_score=50,
            audio_conf=1.0
        )
        ...
        try:
            ...
            if kw not in G.nodes:
                ...
                G.nodes[kw]['memory_score'] = memory_score
                G.nodes[kw]['next_review_time'] = next_review_time
                G.nodes[kw]['last_seen_ts'] = now.isoformat()
                G.nodes[kw]['count'] += 1
        except Exception as e:
    ...
    logger.info(f"OCR pipeline processed {len(keywords_with_counts)} keywords with memory scores.")
