        ...
        try:
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                for concept, info in latest_ocr.items():
                    ...
                    # Memory score weighted by keyword importance and occurrence
                    mem_score = compute_memory_score(
                        last_review, lambda_val, intent_conf, attention_score, audio_conf
                    )
                    mem_score *= min(1.0, kw_score + 0.5)  # boost by keyword score
                    mem_score *= min(1.5, 1 + 0.05 * (count - 1))  # small boost per repeat occurrence
