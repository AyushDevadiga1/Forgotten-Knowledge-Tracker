...
from core.audio_module import record_audio, extract_features as audio_extract_features,audio_pipeline
...
def log_session(window_title, interaction_rate):
    ...
    c.execute("""INSERT INTO sessions 
                 ...
                 VALUES (?, ?, ?, ?, ?)""",
              (ts, ts, app_name, window_title, int(interaction_rate)))  # cast to native int
    conn.commit()
    conn.close()
...
# -----------------------------
# -----------------------------
# Log multi-modal data (patched)
# -----------------------------
def log_multi_modal(window, ocr_keywords, audio_label, attention_score, interaction_rate, intent_label, intent_confidence):
    """
    Logs multi-modal data into the database.
    Ensures all numeric types are native Python types and intent labels are strings.
    """
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    if isinstance(attention_score, (int, float)):
        attention_score = type(attention_score)()
    else:
        attention_score = int(attention_score)

    if isinstance(interaction_rate, (int, float)):
        interaction_rate = type(interaction_rate)()
    else:
        interaction_rate = int(interaction_rate)

    intent_label = str(intent_label)  # ensure string
    intent_confidence = float(intent_confidence)  # ensure float
...
def classify_audio_live():
    ...
    if audio_clf:
        ...
        label = str(audio_clf.predict(features)[0])
...
# -----------------------------
# -----------------------------
# Predict intent (patched)
# -----------------------------
def predict_intent_live(ocr_keywords, audio_label, attention_score, interaction_rate):
    """
    Predicts intent using trained classifier.
    Falls back to rule-based prediction if classifier is unavailable.
    Always returns string intent_label and float confidence.
    """
    ...
    if intent_clf and intent_label_map:
        ...
        if isinstance(idx, int):
            label = intent_label_map.get(idx, "unknown")
        else:
            label = str(idx)

        return {"intent_label": label, "confidence": float(confidence)}
        ...
        try:
            ...
            if audio_counter >= AUDIO_INTERVAL:
                try:
                    ...
                    print(f"Audio pipeline error (fallback to default): {repr(e)}")
                    latest_audio = "unknown"
                    audio_conf = 0.0
                audio_counter = 0
            ...
            if webcam_counter >= WEBCAM_INTERVAL and USER_ALLOW_WEBCAM:
                try:
                    ...
                    print(f"Faces detected: {latest_attention}")
                except Exception as e:
                    print(f"Webcam error (ignored): {repr(e)}")
                    latest_attention = 0
                webcam_counter = 0
            ...
            try:
                intent_data = predict_intent(
                    ...
                    interaction_rate=int(latest_interaction),
                    use_webcam=USER_ALLOW_WEBCAM
                )
                print(f"Intent -> {intent_data['intent_label']} | Confidence: {float(intent_data['confidence']):.2f}")
            except Exception as e:
                print(f"Intent prediction error (fallback): {repr(e)}")
                intent_data = {"intent_label": "idle", "confidence": 0.5}

            # -------- Log Multi-Modal Data --------
            try:
                # ensure all are native Python types
                log_multi_modal(
                    window,
                    latest_ocr,
                    latest_audio,
                    int(latest_attention),
                    int(latest_interaction),
            ...
            print(f"Unexpected tracker error: {repr(e)}")
            time.sleep(TRACK_INTERVAL)
