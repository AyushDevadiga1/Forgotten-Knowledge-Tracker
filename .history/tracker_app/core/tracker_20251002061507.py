...
def log_multi_modal(window, ocr_keywords, audio_label, attention_score, interaction_rate, intent_label, intent_confidence):
    ...
    # Prepare OCR keywords for JSON storage
    # Example: {"photosynthesis": {"score": 0.85, "count": 3}, ...}
    ocr_data_to_store = {}
    ...
    while True:
        try:
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                latest_ocr = ocr_data.get('keywords', {})  # {kw: {"score": s, "count": n}}
                ocr_counter = 0

                if latest_ocr:
                    add_concepts(list(latest_ocr.keys()))

            # -------- Memory Modeling with OCR weights --------
            G = get_graph()
            for concept, info in latest_ocr.items():
                kw_score = info.get("score", 0.5)
                ...
                # Boost by keyword importance and occurrence
                mem_score *= min(1.0, kw_score + 0.5)          # boost for importance
                mem_score *= min(1.5, 1 + 0.05 * (count - 1)) # boost for repetition

                # Optional: logarithmic scaling for high repetitions to avoid extreme scores
                # mem_score *= min(2.0, 1 + 0.2 * np.log1p(count))
