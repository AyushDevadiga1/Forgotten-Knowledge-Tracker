...
# ✅ session manager
from core.session_manager import create_session, update_session, end_session
...
def track_loop():
    ...
    while True:
        try:
            ...
            # ✅ update session data and auto-save
            update_session(session_data, "window_title", window, save_to_db=True)
            update_session(session_data, "interaction_rate", latest_interaction, save_to_db=True)
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                update_session(session_data, "ocr_keywords", latest_ocr, save_to_db=True)
            ...
            if audio_counter >= AUDIO_INTERVAL:
                ...
                update_session(session_data, "audio_label", latest_audio, save_to_db=True)
            ...
            if webcam_counter >= WEBCAM_INTERVAL and USER_ALLOW_WEBCAM:
                try:
                    ...
                    update_session(session_data, "attention_score", latest_attention, save_to_db=True)
            ...
            update_session(session_data, "intent_label", intent_data['intent_label'], save_to_db=True)
            update_session(session_data, "intent_confidence", intent_data['confidence'], save_to_db=True)
