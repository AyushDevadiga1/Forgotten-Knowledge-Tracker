...
def log_multi_modal(window, ocr_keywords, audio_label, attention_score, interaction_rate, intent_label, intent_confidence):
    ...
    # Cast all values to native Python types
    ...
    while True:
        try:
            ...
            if audio_counter >= AUDIO_INTERVAL:
                ...
                audio_conf = float(audio_conf)  # enforce native float
            ...
            if webcam_counter >= WEBCAM_INTERVAL and USER_ALLOW_WEBCAM:
                try:
                    ...
                    latest_attention = int(num_faces)  # enforce native int
            ...
            intent_data['confidence'] = float(intent_data['confidence'])  # enforce float
