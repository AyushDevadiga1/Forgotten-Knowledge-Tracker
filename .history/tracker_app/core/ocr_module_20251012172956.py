...
def ocr_pipeline() -> Dict[str, Any]:
    ...
    # --- Integrate memory model ---
    last_reviews = [datetime.now()] * len(concepts)  # Placeholder; fetch actual last review from DB
    memory_scores = memory_model.compute_memory_scores_batch(
        concepts, last_reviews
    )

    emb = get_text_embedding(text)
    keywords_with_scores = extract_keywords(text, top_n=15)
    
    # Include memory scores in output
    for kw in keywords_with_scores.keys():
        if kw in memory_scores:
            keywords_with_scores[kw]["memory_score"] = memory_scores[kw]
