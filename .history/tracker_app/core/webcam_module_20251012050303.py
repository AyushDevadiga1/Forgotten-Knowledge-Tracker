...
webcam_module.py
----------------
Handles webcam frame capture, face & eye tracking, head orientation,
occlusion detection, and attentiveness scoring.
Fully structured with scenario-aware outputs.
"""
...
def head_orientation(shape) -> Dict[str, Any]:
    """Estimate head pose to detect attention direction."""
    try:
        # Use simple nose to eyes ratio for rough orientation
        ...
        horizontal_ratio = (nose[0] - left_eye_center[0]) / (right_eye_center[0] - left_eye_center[0])
        ...
        logger.error(f"Head orientation failed: {e}")
...
def webcam_pipeline() -> Dict[str, Any]:
    ...
    frame = enhance_low_light(frame)a
