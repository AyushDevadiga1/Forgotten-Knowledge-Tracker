...
def log_multi_modal(window, ocr_keywords, audio_label, attention_score, interaction_rate, intent_label, intent_confidence):
    ...
    ''', (
        timestamp,
        window,
        json.dumps(ocr_keywords),  # store as dict with scores
        audio_label,
        attention_score,
        interaction_rate,
        intent_label,
...
def track_loop():
    ...
    latest_ocr = {}  # {keyword: {"score": float, "count": int}}
    ...
    while True:
        try:
            ...
            if ocr_counter >= SCREENSHOT_INTERVAL:
                ...
                raw_keywords = ocr_data.get('keywords', {})  # dict {keyword: score}
                latest_ocr = {}

                if raw_keywords:
                    text_lower = ocr_data["raw_text"].lower()
                    for kw, score in raw_keywords.items():
                        count = text_lower.split().count(kw.lower())
                        latest_ocr[kw] = {"score": score, "count": count}

                    add_concepts(list(latest_ocr.keys()))
                ocr_counter = 0

                # -------- Memory Modeling with OCR weights and counts --------
                G = get_graph()
                for concept, data in latest_ocr.items():
                    kw_score = data["score"]
                    kw_count = data["count"]
            ...
            log_multi_modal(
                window,
                latest_ocr,  # dict with score + count
                latest_audio,
                latest_attention,
                latest_interaction,
