# ==========================================================
# core/knowledge_graph.py | IEEE v3.1 Multi-Modal Upgrade
# ==========================================================
"""
Knowledge Graph Management Module (v3.1)
----------------------------------------
- Fully async-ready for multi-modal inputs (OCR, Audio, Intent, Attention)
- Lazy-loading embedding models (SentenceTransformer)
- Incremental & safe node/edge updates with memory & attention metadata
- Node types: Concept, OCRKeyword, Intent, AudioEvent
- Edge types: semantic, co_occurrence, temporal, intent_association
- Graph validation, analytics, and full logging
- Stale node pruning & centrality computation
"""
...
from config import DB_PATH, GRAPH_PATH, GRAPH_SIMILARITY_THRESHOLD, STALE_NODE_DAYS
    ...
    try:
        ...
        for concept, mem_score, ts in rows:
            if concept in knowledge_graph:
                node = knowledge_graph.nodes[concept]
                node['memory_score'] = mem_score
                node['last_seen_ts'] = ts or now_iso
            else:
                knowledge_graph.add_node(
                    concept,
                    type="Concept",
