...
def maybe_notify(concept: str, memory_score: float, graph) -> None:
    now = datetime.now()
        ...
        while not stop_event.is_set():
            try:
                ...
                if audio_counter >= AUDIO_INTERVAL:
                    ...
                    logging.debug("Audio -> %s | conf=%.3f", latest_audio, audio_conf)
                    audio_counter = 0
                ...
                if ocr_counter >= SCREENSHOT_INTERVAL:
                    try:
                        ...
                        if latest_ocr:
                            logging.debug("OCR keywords: %s", list(latest_ocr.keys()))
                            add_concepts(list(latest_ocr.keys()))
                        else:
                            logging.debug("OCR: none")
                    except Exception:
                    ...
                    # memory & reminders per detected concept
                    for concept, info in latest_ocr.items():
                        try:
                            ...
                            logging.exception("Error updating memory for concept '%s'", concept)
                ...
                if webcam_counter >= WEBCAM_INTERVAL and USER_ALLOW_WEBCAM:
                    try:
                        ...
                        logging.debug("Faces detected: %s", num_faces)
                    except Exception:
                        logging.exception("Webcam pipeline failed.")
                        latest_attention = 0
                    webcam_counter = 0
                ...
                logging.debug("Intent -> %s | conf=%.3f", intent_data.get("intent_label"), intent_data.get("confidence", 0.0))

                # Add co-occurrence edges (safe)
                try:
                    add_edges(latest_ocr, latest_audio, intent_data.get("intent_label", "unknown"))
                except Exception:
                    logging.exception("Failed to add co-occurrence edges.")

                # -------- Multi-modal logging --------
                try:
                    memory_score = max([float(G.nodes[k].get("memory_score", 0.0)) for k in latest_ocr.keys()] or [0.0])
                    log_multi_modal(
                        window,
                        latest_ocr,
                        latest_audio,
                    ...
                    logging.exception("Failed multi-modal log.")
                ...
                logging.exception("Unexpected loop error; sleeping and continuing.")
                time.sleep(TRACK_INTERVAL)
    finally:
        try:
            save_knowledge_graph()
        except Exception:
            logging.exception("Failed to save graph on shutdown.")
        try:
            kb_listener.stop()
            ms_listener.stop()
        except Exception:
