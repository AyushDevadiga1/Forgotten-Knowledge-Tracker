# ==========================================================
# core/ocr_module.py | FKT v4.0 Async & Multi-Modal
# ==========================================================
"""
OCR Module (IEEE-Ready Async v4.0)
----------------------------------
- Async screenshot capture with optional ROI
- Safe preprocessing & OCR extraction
- Keyword extraction + memory scoring
- Knowledge graph integration + metrics table
...
from config import TESSERACT_PATH, FALLBACK_EMBEDDING_DIM
...
def get_embedding_model():
    ...
    if _embedding_model: return _embedding_model
    try:
...
def extract_text(img: Optional[np.ndarray]) -> str:
    if img is None or pytesseract is None: return ""
    try:
        return str(pytesseract.image_to_string(img, config='--oem 3 --psm 6'))
    except Exception as e:
...
def extract_keywords(text: str, top_n: int = 15) -> Dict[str, Dict[str, Any]]:
    ...
    if kw_model:
        try:
            ...
            for k, score in kws: kw_dict[k.lower()] = float(score)
        except Exception as e:
    ...
    if nlp:
        try:
            ...
            for kw in nlp_kws + entities:
                if kw and kw not in kw_dict: kw_dict[kw] = max(kw_dict.get(kw,0.0),0.3)
        except Exception as e:
            logger.warning(f"spaCy augmentation failed: {e}")
    if not kw_dict:
        tokens = [t.lower() for t in re.findall(r"[A-Za-z]{3,}", text)]
        freq = {}
        for t in tokens: freq[t] = freq.get(t,0)+1
        for k,v in sorted(freq.items(), key=lambda x: x[1], reverse=True)[:top_n]: kw_dict[k]=float(v)/max(1,len(tokens))
    # Knowledge graph boost
    try:
        G = get_graph()
        for kw in list(kw_dict.keys()):
            if G and hasattr(G,'nodes') and kw in G.nodes: kw_dict[kw] += 0.2
    except Exception as e:
        logger.warning(f"KG boost failed: {e}")
...
def get_embeddings(text: str) -> np.ndarray:
    if not text: return np.zeros(FALLBACK_EMBEDDING_DIM)
    model = get_embedding_model()
    if not model: return np.zeros(FALLBACK_EMBEDDING_DIM)
    try:
...
async def ocr_pipeline(roi: Optional[Tuple[int,int,int,int]] = None) -> Dict[str, Any]:
    ...
    # Compute memory scores async
    keywords_with_counts: Dict[str, Dict[str, Any]] = {}
    now = datetime.now()
    G = get_graph()
    text_lower = text.lower() if text else ""

    for kw, meta in keywords_raw.items():
        ...
        try:
            ...
            logger.warning(f"KG update failed for '{kw}': {e}")

        keywords_with_counts[kw] = {
            "score": score,
            "count": count,
...
if __name__ == "__main__":
    ...
    print("Concepts:", result.get("concepts_v2"))
