# core/reminders.py
"""
Reminder Management Module (IEEE-Ready, Unified Logging)
-------------------------------------------------------
...
- Uses centralized DB logging via `db_module`.
"""
...
from core.db_module import init_multi_modal_db, log_multi_modal_event  # centralized logging
...
def check_reminders() -> int:
    ...
    - Logs reminders to DB via centralized logger
    Returns:
        ...
        for node in list(G.nodes):
            ...
            if due_for_review and cooldown_passed:
                ...
                if send_notification(node, mem_score):
                    ...
                    # Log reminder via centralized DB logger
                    log_multi_modal_event(
                        window_title=f"Reminder: {node}",
                        ocr_keywords=node,
                        audio_label=None,
