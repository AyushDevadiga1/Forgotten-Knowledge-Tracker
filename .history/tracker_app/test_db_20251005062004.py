...
# -----------------------------
def clean_table(conn, table_name, check_func):
    ...
    if not invalid_rows.empty:
        ...
        c = conn.cursor()
        if "id" in invalid_rows.columns:
            ids = invalid_rows["id"].tolist()
            c.execute(f"DELETE FROM {table_name} WHERE id IN ({','.join(map(str, ids))})")
        else:
            # fallback for tables without id
            for idx in invalid_rows.index:
                c.execute(f"DELETE FROM {table_name} WHERE rowid = ?", (idx+1,))
        conn.commit()
    else:
...
# -----------------------------
# Run full DB check and cleaning
# -----------------------------
def clean_db():
    ...
    for table, check_func in tables_checks.items():
        total, valid, invalid = clean_table(conn, table, check_func)
