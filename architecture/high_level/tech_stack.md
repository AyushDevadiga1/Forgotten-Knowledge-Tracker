...
## Core Libraries
- **Flask (3.1.x)** & **Flask-SocketIO**: For lightweight synchronous and asynchronous web endpoints.
...
- **Monolithic Modularity**: A single entry point (`main.py`) orchestrates distinct directories for `/tracking`, `/learning`, and `/web`, all operating on a shared, unified `/data/sessions.db` SQLite volume.
