...
- **`tab_relevance` uses a concept-coverage matcher, not bare token co-occurrence.** Measured against the real golden concept trove, single-token co-occurrence makes BOTH hub pages score 1.0 (012 shares "kaggle"; 014 shares "google" with the "google cloud" concept) - the gate would not fix its headline case. The tab carries 1-2 short phrases, so `compute_tab_relevance` matches a phrase to a concept only when the phrase covers the concept's token set (full set for single-word concepts like "kaggle"/"leetcode", >= 2 tokens for longer ones). This keeps 014 at 0.0 (demotes, fixing it), keeps 012 at 1.0 (documented hub-page limit), and keeps genuine study tabs at 1.0 (promotes). Still deterministic lexical overlap, no similarity machinery.
