...
- capture every fix in an OpenSpec change first via the `openspec-propose` skill (or `/opsx-propose`), then implement the fix as part of that change.
...
Record exact changed files, reasoning, verification performed, and the OpenSpec change name.
